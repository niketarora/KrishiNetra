...
import { USERS, AI_MODELS, SERVICES, TREND, CROP_MIX, STATS, NOTIFICATIONS } from "../../data/demoData";
import { Users, Cpu, Satellite, Server, Bell, Check } from "../shared/icons";
import { Page, PageHeader, SubNav, Card, Badge, Stat } from "../shared/ui";
          ...
          <tbody>
            {USERS.map((u) => (
              <tr key={u.id} className="border-b border-slate-50 hover:bg-slate-50/60">
                ...
                <td className="px-6 py-3 text-slate-500">{u.region || u.location}</td>
                <td className="px-6 py-3"><Badge color={u.status === "Active" ? "green" : "slate"}>{u.status}</Badge></td>
                <td className="px-6 py-3 text-slate-400">{u.last || "Today"}</td>
              </tr>
            ))}
          </tbody>
        </table>
          ...
          <Card key={m.name} className="p-5 flex items-center gap-4">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="font-mono text-xs text-slate-400">{m.version || "v1.2"}</span>
                <Badge color={m.status === "Production" || m.status === "Deployed" ? "green" : "amber"}>{m.status}</Badge>
              </div>
              <div className="text-sm text-slate-500 mt-0.5">{m.type || m.kind} · updated {m.updated || "Aug 2026"}</div>
            </div>
  ...
  return (
    <>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-6">
        ...
        <Stat label="Last Sync" value="Live" sub="2026" Icon={Check} accent="text-forest-500" />
      </div>
      ...
      <Card className="p-6">
        ...
        <ResponsiveContainer width="100%" height={240}>
          <PieChart>
            <Pie data={CROP_MIX} dataKey="pct" nameKey="name" cx="50%" cy="50%" outerRadius={90} innerRadius={50}>
...
// ---------- System Health ----------
function HealthTab() {
  const online = SERVICES.filter((s) => s.status === "Online" || s.status === "Operational").length;
  return (
    <>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-6">
        ...
        <Stat label="Active Alerts" value="0" sub="All nominal" accent="text-forest-500" Icon={Bell} />
      </div>
            ...
            <tbody>
              {SERVICES.map((s) => (
                <tr key={s.name} className="border-b border-slate-50">
                  ...
                  <td className="px-6 py-3"><Badge color={s.status === "Online" || s.status === "Operational" ? "green" : "amber"}><span className={`w-2 h-2 rounded-full ${s.status === "Online" || s.status === "Operational" ? "bg-forest-500" : "bg-amber-500"}`} />{s.status}</Badge></td>
                  <td className="px-6 py-3 text-slate-600">{s.uptime || "99.9%"}</td>
